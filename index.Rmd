...
p2 <- ggplot(df21, aes(x=faculty_names, y=value,  fill=Year)) + geom_bar(stat="identity", position="dodge") +  scale_fill_manual(values=rev(vec1p))+theme( axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + labs(x="", y="Number of students") + scale_x_discrete(labels = function(x) str_wrap(x, width = 10))
ggplotly(p2)
```
...
p2 <- ggplot(df31, aes(x=faculty_names, y=value,  fill=Year)) + geom_bar(stat="identity", position="dodge") +  scale_fill_manual(values=rev(vec2g))+theme( axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + labs(x="", y="Number of students") + scale_x_discrete(labels = function(x) str_wrap(x, width = 10))
ggplotly(p2)
```
...
p10 <- ggplot(dfqual2, aes(x=Year, y=value,  fill=Degree)) + geom_bar(stat="identity", position="dodge") + scale_fill_viridis_d(option = "plasma") + labs( y="Number of academics")
ggplotly(p10)
